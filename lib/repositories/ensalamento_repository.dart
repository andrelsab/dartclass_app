...
class EnsalamentoRepository {
  ...
  final filtrado = data.where((item) {
  ...
  Future<List<Ensalamento>> buscarTodosEnsalamentos() async {
    ...
    final data = response.data as List<dynamic>;
    return data.map((e) => Ensalamento.fromMap(e as Map<String, dynamic>)).toList();
  }
  ...
  Future<List<Ensalamento>> buscarPorBloco(String bloco) async {
    ...
    if (response.status != 200 && response.status != 201) {
      throw Exception('Erro ao buscar ensalamentos por bloco: status ${response.status}');
    }
    ...
    Future<List<Ensalamento>> buscarEnsalamentosPorDiaEBloco(String dia, String? bloco) async {
    final response = await client
        .from('ensalamentos')
        .select('''
          id,
